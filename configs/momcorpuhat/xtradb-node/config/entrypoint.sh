...
if [ -n "$PXC_SERVICE" ]; then
	...
	curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/queue/$CLUSTER_NAME" -XPOST -d value="$NODE_IP" -d ttl=60

	#get list of IP from queue
	i=$(curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/queue/$CLUSTER_NAME" | jq -r '.node.nodes[].value')
	...
	curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/$CLUSTER_NAME/$NODE_IP/ipaddr" -XPUT -d value="$NODE_IP" -d ttl=30
	curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/$CLUSTER_NAME/$NODE_IP/hostname" -XPUT -d value="$HOSTNAME" -d ttl=30
	curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/$CLUSTER_NAME/$NODE_IP" -XPUT -d ttl=30 -d dir=true -d prevExist=true

	i=$(curl --key /etc/docker/ssl/calculonc-key.pem --cert /etc/docker/ssl/calculonc.pem --cacert /etc/docker/ssl/ca.pem "$DISCOVERY_SERVICE/v2/keys/pxc-cluster/$CLUSTER_NAME/?quorum=true" | jq -r '.node.nodes[]?.key' | awk -F'/' '{print $(NF)}')
	# this remove my ip from the list
	i2="${i[@]//$NODE_IP}"
	...
	/usr/bin/clustercheckcron "${CLUSTERCHECK_USER}" "${CLUSTERCHECK_PASSWORD}" 1 /var/lib/mysql/clustercheck.log 1 &
